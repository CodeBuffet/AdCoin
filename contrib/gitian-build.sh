...
VERSION=
commit=false
url=https://github.com/CodeBuffet/AdCoin
proc=2
mem=2000
lxc=true
...
-u|--url	Specify the URL of the repository. Default is https://github.com/AdCoin-project/AdCoin
...
then
	# Make output folder
	mkdir -p ./AdCoin-binaries/${VERSION}
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../AdCoin/contrib/gitian-descriptors/gitian-win-signer.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-win-signed --destination ../gitian.sigs.ltc/ ../AdCoin/contrib/gitian-descriptors/gitian-win-signer.yml
	    mv build/out/AdCoin-*win64-setup.exe ../AdCoin-binaries/${VERSION}
	    mv build/out/AdCoin-*win32-setup.exe ../AdCoin-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../AdCoin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-osx-signed --destination ../gitian.sigs.ltc/ ../AdCoin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    mv build/out/AdCoin-osx-signed.dmg ../AdCoin-binaries/${VERSION}/AdCoin-${VERSION}-osx.dmg
	fi
	popd
