...
#if defined(NDEBUG)
# error "AdCoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "AdCoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s ACC additional fees, %d delta bytes\n",
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    if (fCheckPOW && !CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams)) {
      printf("doS: %d\n", state.DoS(25, false, REJECT_INVALID, "high-hash", false, "proof of work failed"));
      return state.DoS(25, false, REJECT_INVALID, "high-hash", false, "proof of work failed");
    }
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // AdCoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
